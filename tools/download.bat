...
curl %CURLOPTS% %QTURL% 
7z %ZOPTS% qtbase-opensource-src-%QTVER%.7z || exit /b %errorlevel%

echo Downloading %QTEXTRAURL%
curl %CURLOPTS% %QTEXTRAURL%
7z %ZOPTS% qtwinextras-opensource-src-%QTVER%.7z || exit /b %errorlevel%

echo Downloading %ICUURL%
curl %CURLOPTS% %ICUURL%
7z %ZOPTS% icu4c-%ICUVER_UNDERSCORE%-src.zip || exit /b %errorlevel%
rd /S /Q icu-%ICUVER% ||  exit /b %errorlevel%
